...
function getImageSize(enabledElement) {
  if (enabledElement.viewport.rotation === 0 || enabledElement.viewport.rotation === 180) {
    return {
      width: enabledElement.image.columns,
      height: enabledElement.image.rows
    };
  }  // 修改过 enabledElement.image的width 变成columns，height改成rows

  return {
    width: enabledElement.image.rows,
    height: enabledElement.image.columns
  };
}
